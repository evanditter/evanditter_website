...
const HomePage = () => {
  ...
  const sectionImages = {
    projects: '/src/images/coding_projects_image.jpg', // Updated to use coding projects image
    courses: '/src/images/courses_home_page.png', // Updated to use courses home page image
    tutoring: '/src/images/evan_professional_headshot.jpeg', // You can replace with actual tutoring image
    contact: '/src/images/contact_me.png' // Updated to use contact me PNG image
  };
    ...
    if (!isHovered) {
      const interval = setInterval(() => {
        setHoveredSection(current => {
          const currentIndex = navigationSections.findIndex(section => section.id === current);
      ...
      <div className="py-16">
        <div className="grid md:grid-cols-2 gap-3 items-center">
          ...
          <div className="relative h-[600px] rounded-lg overflow-hidden shadow-lg">
            <div className="absolute inset-0 transition-opacity duration-500 cursor-pointer" onClick={() => window.location.href = navigationSections.find(section => section.id === hoveredSection)?.path || '/projects'}>
              <img
              ...
              <div className="absolute bottom-4 left-4 text-white">
                <h3 className="text-2xl font-bold capitalize">{hoveredSection}</h3>
                <p className="text-sm opacity-90">
                  {hoveredSection === 'projects' && 'Explore my development projects'}
                  {hoveredSection === 'courses' && 'Learn programming with Python'}
